...
public class GeneratePayslip extends JFrame implements ActionListener {
	Choice c1;
	JTextArea t1;
	JButton b1;
	
	GeneratePayslip(){
		super("Generate Payslip of Employees");
		setSize(800,700);
        setLocation(400,150);
        setVisible(true);
        c1 = new Choice();
	...
	public void actionPerformed(ActionEvent ae) {
		try {
	        ...
	        ResultSet rs = c.s.executeQuery("select * from tblEmployee where id="+c1.getSelectedItem());
	        rs.next();
	        String name = rs.getString("name");
	        rs.close();
	     
	        rs = c.s.executeQuery("select * from tblSalary where id="+c1.getSelectedItem());
	        double gross=0;
	        double net=0;

	        java.util.Date d1 = new java.util.Date();
	        int month = d1.getMonth();
	        t1.setText(" ----------------   PAY SLIP FOR THE MONTH OF "+month+" ,2029  ------------------------");
	        t1.append("\n");
